...
const rollDiceLogic = (input: string, name: string) => {
    ...
    if (input.indexOf('r ') != -1) {
        input = input.substring(input.toLowerCase().indexOf('r ') + 2);
    }
    //remove "/" if present
    if (input.startsWith('/')) {
    ...
    //split add operation if any
    if (input.indexOf('+') != -1) {
        addVector = input.split('+');
        input = addVector[0];
    } else {
        addVector = [];
    }
    ...
    if (addVector.length > 1) {
        for (let i = 1; i < addVector.length; i++) {
            if( !isNaN(parseInt(addVector[i], 10)) ){
                sum += parseInt(addVector[i], 10);
            }
        }
    }
